...
def is_skip_keywords(line):
    ...
    skip_keywords = [
        ...
        "help",
        "Pattern attributes",
        "fast NAT", "fast filtering", "fast init", "fast_io", "fast-read", "(fast) read", "--fast", "too fast", "Speicher fast voll", "fast restarting"
        "Aver depth", "depth:"
    ]
    for sk in skip_keywords:
        sk_l = sk.lower()
        sk_u = sk.upper()
        if sk in line or sk_l in line or sk_u in line:
            return True
    return False
...
def is_with_underline(line, info):
    s = "({}|{}|{}|{})[_]".format(info, info.upper(), info.lower(), info.capitalize())
    pattern = re.compile(s.encode())
    temp = pattern.findall(line)
    if len(temp) == 0:
        return False
...
def extra_models_match(file_name, models):
    ans = []
    for model in models:
        m_l = model.lower()
        m_u = model.upper()
        if model in file_name or m_l in file_name or m_u in file_name:
            ans.append(model)
    if len(ans) == 1:
        return True, ans[0]
    else:
...
def more_vendor_extra_models_match(may_info, file_name):
    ans = {}
    for vendor in may_info.keys():
        models = may_info[vendor]
        flag, model_name = extra_models_match(file_name, models)
        if flag:
